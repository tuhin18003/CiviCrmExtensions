...
//        console.log( CRM.searchBuilder.stringFields );
        
      if ($.inArray(field, CRM.searchBuilder.stringFields ) >= 0) {
        // string operators
        buildOperator(operator, CRM.searchBuilder.stringOperators);
      } else if( field === 'prox_distance' ) {
        // custom operators
        buildOperator(operator, prox_distance_operator() );
      } else {
        // general operators
  ...
  /**
   * Build Custom operator for proximity search
   * 
   * @returns {SearchBuilderExt_L2.prox_distance_operator.objOp}
   */
  function prox_distance_operator(){
      var objOp = {
            ''   : "-operator-", 
            '='  : "=", 
            '!=' : "≠", 
  ...
  function initialize() {
    $('.crm-search-block', '#SearchBuilderExt').each(function(blockNo) {
      var block = $(this);
  ...
  $(function($) {
    $('#crm-main-content-wrapper')
      ...
      .on('click', '#addBlock', function() {
        if ($('.crm-search-block:hidden', '#SearchBuilderExt').length) {
          var block = $('.crm-search-block:hidden', '#SearchBuilderExt').first();
          block.show();
      ...
      .on('crmLoad', function() {
        initialize();
        $('select[id^=mapper][id$="_1"]', '#SearchBuilderExt').each(handleUserInputField);
      });
    ...
    $('select[id^=mapper][id$="_1"] option:selected', '#SearchBuilderExt').each(function() {
